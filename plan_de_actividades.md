Actividades principales del proyecto 

-Levantamiento y análisis de requisitos del sistema.
-Elaboración de casos de uso y diagramas UML.
-Diseño de la arquitectura del software y base de datos.
-Desarrollo e implementación de los módulos principales del sistema.
-Pruebas funcionales y corrección de errores.
-Documentación y entrega final del proyecto.


▪ Fechas tentativas de ejecución

-Levantamiento de requisitos: 10/11/2025 – 15/11/2025
-Diseño y casos de uso: 16/11/2025 – 22/11/2025
-Arquitectura y base de datos: 23/11/2025 – 30/11/2025
-Desarrollo del sistema: 01/12/2025 – 20/01/2026
-Pruebas y mejoras: 21/01/2026 – 30/01/2026
-Documentación y entrega: 01/02/2026 – 05/02/2026


▪ Responsables (puedes asignarte uno o varios roles)

Juan David: Analista de requisitos, arquitecto de software y desarrollador, diseñador UX/UI, desarrollador y encargado de pruebas.


▪ Relación con los entregables del diseño del software

-Documento de requisitos funcionales y no funcionales.
-Diagramas UML (casos de uso, clases, actividades).
-Modelo entidad–relación y estructura de base de datos.
-Arquitectura de software definida.
-Prototipo de interfaz de usuario (UI/UX).
-Código fuente funcional y documentado.
-Casos de prueba y resultados de pruebas.
-Documentación técnica y manual de usuario.
